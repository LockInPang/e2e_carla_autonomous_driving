...
class RunCARLA():
    ...
    def process_image(self, image):
        try:
            ...
            steering_angle = self.driving_model.forward(image.reshape(-1, IM_HEIGHT, IM_WIDTH, 3))

            # print(f"Steering: {steering_angle}, Throttle: {throttle_press}, Brake: {brake_pressure}")
            print(f"raw_data:{steering_angle}")
            print(f"Steering: {100*steering_angle[0][0]}")
            control = carla.VehicleControl(throttle=0.4, steer=float(100*steering_angle[0][0]))
    ...
    def run(self, world):
        ...
        while True:
            try:
                ...
                spectator.set_transform(carla.Transform(spectator_transform.location + carla.Location(z=10),
...
if __name__ == "__main__":
    ...
    driving_model = model.PilotNet(IM_WIDTH, IM_HEIGHT, model_file=args.model_path)
